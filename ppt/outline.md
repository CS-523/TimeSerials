...
### 1.2 任务目标
1. 数据清洗：异常值检测、缺失值处理
2. **预测模型**：给定历史 `x1..x8` 窗口，预测未来 `x1..x8` + `y1..y4`；输出长度 ≥ 6，**支持迭代生成**
...
```
┌────────────┐   ┌────────────┐   ┌─────────────┐   ┌──────────────┐
│ raw CSVs   │ → │ preprocess │ → │ analysis    │ → │ SS-NN model  │
│ (1/..5/)   │   │ align, fill│   │ corr, MI,   │   │ hybrid pred. │
└────────────┘   │ scale, split│   │ PCA, lag   │   │              │
                 └────────────┘   └─────────────┘   └──────────────┘
                       │                  │                  │
                       ▼                  ▼                  ▼
                  data/processed/    figures/*.png     checkpoints/*.pt
                  train.npz,
                  test.npz
```
...
| 可视化 | `src_control/visualization/plots.py` | 预测叠加 |
| 驱动脚本 | `scripts_control/*.py` | 各阶段的 CLI 入口 + 端到端冒烟测试 |
...
4. **x7 单调递增**：应保持累积趋势
5. **x3 衰减模式**：从 ~100 衰减到 ~70
6. **混合 SS + NN**：线性 SS 提供稳定基线 (R² ~0.85)，NN 残差补充非线性（最终 R² ~0.97+）

### 4.2 待改进 / 拓展方向
1. **Mamba / GRU 替换 MLP**：长序列（320 步）下，门控 RNN 可能更稳定
2. **ILC（迭代学习控制）**：171 次实验可视为重复任务，跨实验累积改进控制策略
3. **NN 正则化**：用线性 SS 输出作为 NN 残差的先验（贝叶斯风格）
