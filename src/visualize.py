...
def build_test_loader(test_exps, x_scaler: Scaler, seed: int):
    """构造与训练一致的 groupwise test loader（带 group_id）。"""
    samples = sample_windows(test_exps, rng_seed=seed + 2)
    m = {i: int(e.group) - 1 for i, e in enumerate(test_exps)}
    for s in samples:
        s.group = m[s.exp_id]
...
def main():
    ...
    exps = load_all(base)
    train_exps, _, test_exps = split_experiments_groupwise(exps, seed=args.seed)
    if x_scaler is None:
    ...
    # 1) 真实 vs 预测的 x1-x8（每 group 各取最长的一条 test 实验）
    viz_exps = []
    for g in ["1", "2", "3", "4", "5"]:
        gs = [e for e in test_exps if e.group == g]
        if gs:
            viz_exps.append(max(gs, key=lambda e: len(e)))
    print(f"[viz] 可视化实验: {[f'G{e.group} {os.path.basename(e.file)}' for e in viz_exps]}")
    ...
    for ei, exp in enumerate(viz_exps):
        ...
        pred_x = pred_full[pred_start - in_len:]  # 只取预测区间那 T_out 步
        x_future = exp.df[X_COLS].iloc[pred_start:pred_start + T_out].to_numpy(dtype=np.float32)
        print(f"[viz] G{exp.group} {os.path.basename(exp.file)}: input 0-{in_len - 1}, pred {pred_start}-{pred_start + T_out - 1}")
    ...
    plt.tight_layout()
    tag = f"{args.backbone}_{args.mode}"
    ...
    test_loader = build_test_loader(test_exps, x_scaler, args.seed)
    metrics = evaluate_multigroup(model, test_loader, device, args.mode, x_scaler,
    ...
    axes[0].set_title(f"Per-dim RMSE(x), normalized space  (mean={metrics['rmse_x_norm']:.4f})")
    axes[0].set_ylabel("RMSE")
    axes[0].grid(True, alpha=0.3, axis="y")
    axes[1].bar(X_COLS, per_dim_orig, color="darkorange")
    axes[1].set_title(f"Per-dim RMSE(x), original space  (mean={metrics['rmse_x_orig']:.2f})")
